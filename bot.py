...
def help_command(message):
    bot.send_message(
        ...
        "Guruhda:\n"
        "• «Aizo» deb chaqirsangiz ishlaydi\n"
        "• Yoki mening javobimga Reply qilib davom ettirasiz"
    )
...
def reply(message):
    ...
    # =====================================================
    if not is_private:
        # 1. "Aizo" deb chaqirilganmi?
        called_aizo = "aizo" in user_text.lower()

        # 2. Aizo'ning javobiga Reply qilinganmi?
        is_reply_to_bot = (
        ...
        # Ikkalasidan biri bo'lmasa — e'tibor bermaymiz
        if not (called_aizo or is_reply_to_bot):
            return
